...
class BuyOfferWrongPinTestCase(TestCase):
    def __init__(self):
        super().__init__("BuyOfferWrongPinTestCase")
    ...
    def run(self, driver: webdriver) -> bool:
        ...
        if pin_form is None:
            ...
            return False
        pin_form.clear()
        pin_form.send_keys(config.INCORRECT_PIN)
        ...
        notification = utils.wait_for_xpath(driver, config.NOTIFICATION_XPATH, 4)
        ...
        try:
            ...
            return False
        if text != config.EXPECTED_BUY_ERROR_TEXT:
